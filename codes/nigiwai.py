...
D = dist(X[:,args.start_frame],Y[:,args.start_frame],D,args.alpha)
D[:,roi_ids]=np.inf
for fr in range(args.start_frame+1,args.end_frame):
#    print("Computing frame: ",fr)
    ...
    V=np.abs(np.where(np.logical_and(nD != np.inf, D != np.inf), nD-D, np.inf))
...
plt.plot(np.arange(args.start_frame+1,args.end_frame), np.array(Ped_nigiwai))
plt.plot(np.arange(args.start_frame+1,args.end_frame), np.array(ROI_nigiwai))
